...
const ItemSidebar = ({funcs, handleRedirect}: {funcs: any, handleRedirect: (path: string) => void}) => {
  ...
  return(
    <li className='w-full'>
      <div
        className={`grid grid-flow-col gap-x-4 justify-start ${isActive ? 'text-[#FFFFFF]' : 'text-gray-300'} cursor-pointer transition ease-in hover:text-white`}
      ...
      >
        { isActive ?
          ...
          : 
          <>
            <i className={`${icon} pt-[1px] text-[18px]`}/>
            <p className='font-semibold text-[16px]'>{text}</p>
          </>
        }
      </div>
...
const Sidebar = () => {
  ...
  return (
    <div className='w-[265px] h-full bg-black select-none'>
