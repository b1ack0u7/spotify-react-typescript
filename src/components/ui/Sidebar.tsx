...
const ItemSidebar = ({funcs, handleRedirect}: {funcs: any, handleRedirect: (path: string) => void}) => {
  ...
  return(
    <li className='w-full'>
      <div
        className={`flex items-center gap-x-4 ${isActive ? 'text-[#FFFFFF]' : 'text-gray-300'} cursor-pointer transition ease-in hover:text-white`}
        onClick={() => handleRedirect(redirectTo)}
      >
        { isActive ?
          <>
            <i className={`${selectedIcon} pt-[1px] text-[20px]`}/> 
            <p className='font-semibold text-[14px] mb-1'>{text}</p>
          </>
          : 
          <>
            <i className={`${icon} pt-[1px] text-[20px]`}/>
            <p className='font-semibold text-[14px] mb-1'>{text}</p>
          </>
        }
      </div>
...
const Sidebar = ({setShowMenu}: {setShowMenu?: React.Dispatch<React.SetStateAction<boolean>>}) => {
  ...
  const sideBarItems: {name: string, path: string, icon: string}[] = [
    {
      name: 'Inicio',
      path: 'home',
      icon: 'home'
  ...
  return (
    <div className='mx-6 mt-6'>
      <div
        className='flex h-[40px] mb-8 justify-center cursor-pointer'
      ...
      <ul className='grid gap-y-2 text-gray-400 font-medium text-[14px]'>
        <li>GYM</li>
        <li>Adrenaline Workout</li>
        <li>Neonn</li>
      </ul>
    </div>
  )
